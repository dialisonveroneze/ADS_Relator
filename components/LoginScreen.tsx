...
const LoginScreen: React.FC = () => {
    ...
    const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${GOOGLE_CLIENT_ID}&redirect_uri=${encodeURIComponent(googleRedirectUri)}&response_type=code&scope=${encodeURIComponent(scope)}&access_type=offline&prompt=consent`;

    const handleGoogleClick = (e: React.MouseEvent) => {
        if (GOOGLE_CLIENT_ID.includes('INSIRA_SEU_CLIENT_ID')) {
            e.preventDefault();
            alert("Erro de Configuração: Configure o GOOGLE_CLIENT_ID.");
        }
    };
             ...
             <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl max-w-2xl w-full">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <a
                        ...
                        className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-600 font-bold py-3 px-6 rounded-lg text-lg transition-transform transform hover:scale-[1.02] duration-300 ease-in-out flex items-center justify-center gap-3 shadow-sm"
                    >
